...
class AlexNetFc(nn.Module):
  ...
  def get_parameters(self, trial=0):
    if self.new_cls:
        if self.use_bottleneck:
            if trial == 0:
...
class ResNetFc(nn.Module):
  ...
  def get_fc_layers(self, trial=0):
    if self.new_cls:
        if self.use_bottleneck:
            if trial ==0:
                parameter_list = [
...
class ResNet_fewshot(nn.Module):
    def __init__(self, resnet_name, use_bottleneck=True, bottleneck_dim=256, new_cls=False, class_num=1000):
        super(ResNet_fewshot, self).__init__()
        model_resnet = resnet_dict[resnet_name](pretrained=True)
        self.conv1 = model_resnet.conv1
        self.bn1 = model_resnet.bn1
        self.relu = model_resnet.relu
        ...
        if new_cls:
            if self.use_bottleneck:
                self.fc = nn.Linear(model_resnet.fc.in_features, bottleneck_dim)
                self.fc2 = distLinear(bottleneck_dim, class_num)
                self.fc.apply(init_weights)
                # self.fc2.apply(init_weights)
                self.__in_features = bottleneck_dim
            else:
                self.fc2 = distLinear(model_resnet.fc.in_features, class_num)
                # self.fc2.apply(init_weights)
                self.__in_features = model_resnet.fc.in_features
        else:
            self.fc = model_resnet.fc
